package org.technoserve.cherieapp.workers
...
import org.technoserve.cherieapp.database.AppDatabase
...
class UploadWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    override fun doWork(): Result {
        val shared_prefs = getSharedPreferences(applicationContext)
        val country_name = readCountry(shared_prefs)
        ...
        if (fileNames != null) {
            ...
            for (i in fileNames.indices) {
                ...
                // if filename is null, make it random
                if (fileName == null) {
                    fileName = "anon_${System.currentTimeMillis()}"
                }
                val imageUri = Uri.parse(imageUris?.get(i))
                val predictionId = predictionIds?.get(i)
                val storageReference =
                    FirebaseStorage.getInstance().getReference("images/${country_name.lowercase()}/$fileName")
                            ...
                            storageReference.downloadUrl.addOnCompleteListener { it2 ->
                                if (it2.isSuccessful) {
                                    ...
                                    val payload = hashMapOf(
                                        ...
                                        "region" to  country_name,
                                        "coordinates" to prediction.coordinates,
                                        "rating" to prediction.rating
                                    )
